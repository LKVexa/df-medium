# DF_Medium (N_MEDIUM) -- capability ledger
...
| `DFN-07` | the VM's own build and acceptance gate | `OPERATIONAL` | `make operational` reproduces from the delivered bytes with stock flags; the VM's own sums verify afterwards (SHA256SUMS (240 entries)) | `N1`=PASS |
...
| `DFN-17` | equivalence with the corpora's adapter | `OPERATIONAL` | lowering byte-identical to pacore.adapters.brlower; identical witness through pacore.adapters.bottlerocket; the corpora's 30-check backend suite passes against this node | `A10`=PASS, `A11`=PASS |
...
| `DFN-34` | the target's own blockers | `BLOCKED` | independent second-machine/operator replay; independent second-machine/operator rebuild; literal 72-hour native soak; AArch64 and Windows execution qualification; final 100% evidence ledger dependent on those proofs | - |
